...
class CarlaManager:
    def __init__(self):
        ...
        self.weather_manager: Optional[weather_manager.WeatherManager] = None
    ...
    def _start_carla_server_process(self, host='127.0.0.1', port=2000) -> bool:
        """
        [수정] Popen 실행 시 CWD(Current Working Directory)를 
        ...
        if self._is_port_in_use(host, port):
            # print(f"[Manager] Port {port} is already in use") # 로그 생략됨
            return True
        ...
        try:
            ...
            self.carla_server_process = subprocess.Popen(
                ...
                cwd=carla_root_dir  # <--- [핵심 수정] 작업 디렉토리를 carla 루트로 설정
            )
    ...
    def connect(self, start_server: bool = True, host='127.0.0.1', port=2000, tm_port=8000):
        ...
        try:
            ...
            self.weather_manager = weather_manager.WeatherManager(self.world)
    ...
    def tick(self) -> bool:
        ...
        try:
            # 동적 날씨가 켜져있으면 틱 실행
            if self.weather_manager and self.weather_manager.is_dynamic:
                delta_seconds = self.world.get_settings().fixed_delta_seconds or 0.05
                self.weather_manager.tick(delta_seconds)
    ...
    def start_manual_control(self) -> Tuple[bool, str]:
        ...
        # 스크립트 경로 확인 (carla_gui.py와 같은 폴더에 있다고 가정)
    ...
    def cleanup(self):
        ...
        (수정됨: carla_gui_1.py의 '단순' 종료 로직으로 복귀 - SIGTERM)
        """
        ...
        # 4. CARLA 서버 프로세스 종료 (단순 .terminate() 사용)
